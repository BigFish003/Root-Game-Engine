...
def _search(engine: Any, depth: int, branch_factor: int) -> float:
    ...
    for action in actions[:branch_factor]:
        ...
        sim._eyrie_ai_enabled = False
        sim.apply_action(action)
        score = _search(sim, depth - 1, branch_factor) + _action_bias(action)
        best = max(best, score)
    return best
...
def _action_bias(action: Any) -> float:
    ...
    if isinstance(action, AddToDecree): return 7
    if isinstance(action, Craft): return 6
    if isinstance(action, EndDecision): return -4
