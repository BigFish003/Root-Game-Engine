...
class VisualRenderer:
    ...
    def _render_clearings(self, payload: "_StatePayload") -> str:
        ...
        for cid in sorted(payload.clearings):
            ...
            blocks.extend(self._render_clearing_stats_panel(clearing, x - 70, y + 44))
        return "\n".join(blocks)

    def _render_clearing_stats_panel(self, clearing: "_ClearingPayload", x: int, y: int) -> list[str]:
        rows: list[str] = [
            f'<rect x="{x}" y="{y}" width="140" height="48" rx="6" fill="#ffffff" stroke="#d5d8dc" />'
        ]
        warrior_row = " ".join(
            f"{faction.value[:1].upper()}:{clearing.warriors.get(faction, 0)}" for faction in Faction
        )
        rows.append(
            f'<text x="{x + 6}" y="{y + 14}" font-size="10" fill="#1f2d3d">W {warrior_row}</text>'
        )
        building_row = self._format_piece_row(clearing.buildings)
        token_row = self._format_piece_row(clearing.tokens)
        rows.append(
            f'<text x="{x + 6}" y="{y + 28}" font-size="10" fill="#1f2d3d">B {escape(building_row)}</text>'
        )
        rows.append(
            f'<text x="{x + 6}" y="{y + 42}" font-size="10" fill="#1f2d3d">T {escape(token_row)}</text>'
        )
        return rows

    def _format_piece_row(self, by_faction: dict[Faction, list[str]]) -> str:
        groups: list[str] = []
        for faction in Faction:
            ...
            groups.append(f"{faction.value[:1].upper()}:{'/'.join(labels)}")
